...
5. **Clean up, then sync + merge + push:**
   ...
   - **Mark the backlog entry done:** if the work came from a `tasks/backlog.md` entry, flip its `_status: …_` to `_status: done (<YYYY-MM-DD>)_` so it isn't re-picked. Standalone doc edit — commit it straight to `develop` and push (the branch is already gone).
